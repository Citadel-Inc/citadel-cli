...
specs/active/, specs/parked/     SDD specs (+ parked = not pursued)
...
| Cut a release | update `CHANGELOG.md`; run `make verify && make build-all VERSION=vX.Y.Z`; tag `vX.Y.Z`; `cli-release.yml` builds + publishes to GH Releases |
