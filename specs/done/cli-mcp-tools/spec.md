...
**HUMAN follow-up.** Positive production smoke (real JWT → `tools/list` → `tools/call get_namespace --arg path=damon`) requires interactive `citadel auth login` against the live Supabase, which the agent cannot drive. Tracked in `specs/HUMAN_BLOCKERS.md` §19 (see also **§08–09** for go-mcp-oauth inspector / consent flows).
