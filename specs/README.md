...
| Slug | DTG | Note |
|------|-----|------|
| cli-self-host-setup | 102042ZMAY26 | P1+P2 residuals complete: --debug flag wired to stderr with secrets redacted, httptest-backed integration tests gated on CITADEL_TEST_SELF_HOST_LIVE, docs/cli.md enriched with troubleshooting section and live test recipe, Q-table ratified. |
...
| cli-auth-providers | 071821ZMAY26 | Shipped `auth provider list|link|unlink` with public provider discovery, browser-default link initiation, provider-ID completion, tests, and docs. make verify passed. P2 live smoke deferred (prod empty `/auth/providers`; citadel#9). |
