...
func TestIssueComment_Happy(t *testing.T) {
	withServer(t, func(w http.ResponseWriter, r *http.Request) {
	...
	if err := rootFor(cmd.IssueCmd, "comment", "-R", "acme/demo", "7", "--body", "looks good").Execute(); err != nil {
		t.Fatal(err)
	}
}

func TestIssueComment_EmptyBody(t *testing.T) {
	err := rootFor(cmd.IssueCmd, "comment", "-R", "acme/demo", "7", "--body", "").Execute()
